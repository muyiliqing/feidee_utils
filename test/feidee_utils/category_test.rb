...
class FeideeUtils::CategoryTest < MiniTest::Test
  ...
  def test_validate_global_integrity_errors_multiple_project_root
    @sqlite_db.execute <<-SQL
      ...
      VALUES(170, 'category1', 0, '/170/', 0, 0, -3, '', 0, 2, 100, -1);
    SQL
    e = assert_raises do
      @sqlite_db.ledger::Category.validate_global_integrity
    end
    assert_equal(
      "More than one category have type project_root. IDs are [-69, 170].",
      e.message
    )
  ...
  def test_validate_global_integrity_errors_unexpected_name
    @sqlite_db.execute <<-SQL
      UPDATE t_category SET name='another_root' WHERE name='root';
    SQL
    e = assert_raises do
      @sqlite_db.ledger::Category.validate_global_integrity
    end
    assert_equal "Category another_root has type project_root. ID: -69.",
      e.message
    @sqlite_db.execute <<-SQL
      UPDATE t_category SET name='root' WHERE name='another_root';
    SQL
  end
