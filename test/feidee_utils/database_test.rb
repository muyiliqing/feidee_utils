...
class FeideeUtils::DatabaseTest < MiniTest::Test
  def setup
    ...
    @fresh_ios_backup = FeideeUtils::Database.open_file(
      base_path.join("../data/QiQi-20150422203954.sqlite")
    )
    @oneline_ios_backup = FeideeUtils::Database.open_file(
      base_path.join("../data/QiQi-20150422204102.sqlite")
    )
    @complex_android_backup = FeideeUtils::Database.open_file(
      base_path.join("../data/Daily_20140401.sqlite")
    )
  end
  ...
  def test_timestamp
    assert_equal Time.parse("1970-01-01 10:00:00 +1000"),
      @fresh_ios_backup.last_modified_at
    assert_equal Time.parse("2015-04-22 20:40:29 +1000"),
      @oneline_ios_backup.last_modified_at
    assert_equal Time.parse("2014-04-01 11:54:44 +1100"),
      @complex_android_backup.last_modified_at
  end
  ...
  def test_open_illegal_private_header
    ...
    assert_match (/^Unexpected private sqlite header .*\./), e.message
  end
  ...
  def test_trash_table_name
    assert_equal 't_record_delete',
      (FeideeUtils::Database.trash_table_name "t_record")
    assert_equal 't_deleted_transaction',
      (FeideeUtils::Database.trash_table_name "t_transaction")
  end
  ...
  def test_validate_global_integrity_errors
    # TODO: The db versions of complex_android_backup and the default iOS test
    # db are different.
    @complex_android_backup.execute <<-SQL
    ...
    @complex_android_backup.execute(
      "DELETE FROM t_account WHERE accountPOID = -1"
    );
  end
end
